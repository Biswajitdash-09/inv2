...
export async function GET(request) {
    try {
        ...
        const allPMs = await User.find({ role: { $in: ['PM', 'Project Manager', 'project manager'] } })
            .lean()
        ...
        const pmResults = await Promise.all(allPMs.map(async (pm) => {
            let manager = null;
            if (pm.managedBy) {
                manager = await User.findOne({ id: pm.managedBy }).lean().select('id name role');
            }
            return {
                pm_id: pm.id,
                pm_name: pm.name,
        ...
        const invoiceDetails = await Promise.all(recentInvoices.map(async (inv) => {
            ...
            if (inv.assignedFinanceUser) {
                const fu = await User.findOne({ id: inv.assignedFinanceUser }).lean().select('name role');
                fuName = fu ? `${fu.name} (${fu.role})` : 'NOT FOUND in DB';
            }
            if (inv.assignedPM) {
                const pm = await User.findOne({ id: inv.assignedPM }).lean().select('name managedBy');
        ...
        const allFUs = await User.find({ role: { $in: ['Finance User', 'finance user', 'Finance_User'] } })
            .lean()
