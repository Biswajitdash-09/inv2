...
export const db = {
        ...
        try {
            ...
            if (user) {
                ...
                if (role === ROLES.PROJECT_MANAGER) {
                    ...
                    // OR invoices whose assigned PM reports directly to them (hierarchy fallback).
                    // The fallback prevents pre-fix invoices from being invisible.
                    const myPMs = await User.find({ managedBy: user.id }).select('id').lean();
                    const myPMIds = myPMs.map(u => u.id);

                    if (myPMIds.length > 0) {
                        query.$or = [
                            { assignedFinanceUser: user.id },
                            { assignedPM: { $in: myPMIds }, assignedFinanceUser: { $in: [null, '', undefined] } }
                        ];
                    } else {
    ...
    getInvoice: async (id, requestingUser = null) => {
        try {
            ...
            // Resolve PM and Finance User names
            const userIdsToResolve = [inv.assignedPM, inv.assignedFinanceUser].filter(Boolean);
            if (userIdsToResolve.length > 0) {
                const users = await User.find({ id: { $in: userIdsToResolve } }).select('id name').lean();
                const nameById = {};
            ...
            if (inv.documents && inv.documents.length > 0) {
                ...
                if (docsWithoutName.length > 0) {
                    const DocumentUpload = (await import('@/models/DocumentUpload')).default;
